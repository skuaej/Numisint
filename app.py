...
# ---------- DuckDB Streaming Setup ----------
con = duckdb.connect()
con.execute("INSTALL httpfs;")
con.execute("LOAD httpfs;")
con.execute("SET enable_http_metadata_cache=true;")
con.execute("SET http_timeout=120000;")  # longer timeout
print("DuckDB ready (streaming mode)")
# -------------------------------------------
...
def fetch_data(Number: str = Query(None)):
    ...
    # Main – only reads the matching row (LIMIT 1)
    try:
        df_main = con.execute(f"""
